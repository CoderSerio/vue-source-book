...
const Sidebar: React.FC<SidebarProps> = ({ currentChapterId, scrollToChapter }) => {
  return (
    <div className="fixed left-6 top-1/2 -translate-y-1/2 z-50 group">
      {/* Collapsed State (Cute Icons) */}
      <div className="flex flex-col gap-4 p-3 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-full transition-all duration-300 group-hover:opacity-0 group-hover:scale-90 pointer-events-auto absolute top-0 left-0 transform -translate-y-1/2">
        {CHAPTERS.map((chap) => (
          <div 
            key={chap.id}
      ...
      <div className="flex flex-col gap-3 bg-[#fffef0] p-6 pr-8 rounded-[2rem] border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] w-80 transition-all duration-500 opacity-0 invisible -translate-x-10 group-hover:translate-x-0 group-hover:opacity-100 group-hover:visible relative -left-4 transform -translate-y-1/2">
        ...
        <div className="pl-10">
            ...
            >
                <span className={`w-6 h-6 rounded-full border border-black mr-3 flex items-center justify-center text-xs flex-shrink-0 ${chap.color}`}>
                    {chap.id}
                </span>
